...
//    public static final String ACTION_DATA_AVAILABLES = ".LeProxy.ACTION_DATA_AVAILABLES";
    ...
    public void setAutoConnect(String address, boolean autoConnect) {
        if (mBleService != null) {
            mBleService.setAutoConnect(address, autoConnect);
        }
    }
    ...
    public boolean enableNotification(String address, UUID serUuid, UUID charUuid) {
        ...
        BluetoothGatt gatt = mBleService.getBluetoothGatt(address);
        BluetoothGattCharacteristic c = GattUtil.getGattCharacteristic(gatt, serUuid, charUuid);
        LogUtils.e(TAG, "gatt=" + gatt + ", characteristic=" + c);
    ...
    public boolean enableNotificationWithoutWriteDescriptor(String address, UUID serUuid, UUID charUuid) {
        BluetoothGatt gatt = mBleService.getBluetoothGatt(address);
        BluetoothGattCharacteristic c = GattUtil.getGattCharacteristic(gatt, serUuid, charUuid);
        LogUtils.e(TAG, "enableNotificationWithoutWriteDescriptor() - gatt=" + gatt + ", characteristic=" + c);
    ...
    private final BleCallBack mBleCallBack = new BleCallBack() {
        ...
        public void onConnected(String address) {
            ...
            LogUtils.i(TAG, "onConnected() - " + address);
        ...
        public void onConnectTimeout(String address) {
            LogUtils.e(TAG, "onConnectTimeout() - " + address);
            updateBroadcast(address, ACTION_CONNECT_TIMEOUT);
        }

        @Override
        public void onConnectionError(String address, int error, int newState) {
            LogUtils.e(TAG, "onConnectionError() - " + address + " error code: " + error + ", new state: " + newState);
        ...
        public void onDisconnected(String address) {
            LogUtils.e(TAG, "onDisconnected() - " + address);
        ...
        public void onServicesDiscovered(String address) {
            ...
            LogUtils.i(TAG, "onServicesDiscovered() - " + address);
            new Timer().schedule(new ServicesDiscoveredTask(address), 300, 100);
        }
        ...
        public void onServicesUndiscovered(String address, int status) {
            //检索服务异常
            LogUtils.e(TAG, "onServicesUndiscovered() - " + address + ", status = " + status);
        }
        ...
        public void onCharacteristicChanged(String address, BluetoothGattCharacteristic characteristic) {
            ...
            LogUtils.i("zhonghuibin", DataUtil.byteArrayToHex(data));
            LogUtils.i(TAG, "onCharacteristicChanged() - " + address + " uuid=" + characteristic.getUuid().toString()
                    + "\n len=" + (data == null ? 0 : data.length) + " [" + DataUtil.byteArrayToHex(data) + ']');
        ...
        public void onCharacteristicRead(String address, BluetoothGattCharacteristic characteristic, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                //读取到模块数据
                LogUtils.i(TAG, "onCharacteristicRead()" + address + " - uuid=" + characteristic.getUuid().toString()
        ...
        public void onCharacteristicWrite(String address, BluetoothGattCharacteristic characteristic, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                LogUtils.i(TAG, "onCharacteristicWrite() - " + address + ", " + uuid
                        + "\n len=" + characteristic.getValue().length
                        + " [" + DataUtil.byteArrayToHex(characteristic.getValue()) + ']');
        ...
        public void onMtuChanged(String address, int mtu, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                LogUtils.i(TAG, "onMtuChanged() - " + address + ", MTU has been " + mtu);
            } else {
                LogUtils.e(TAG, "onMtuChanged() - " + address + ", MTU request failed: " + status);
            }
    ...
    public void updateBroadcasts(String address, byte[] bytes) {
        String dataStr = DataUtil.byteArrayToHex(bytes).replace(" ", "");
        LogUtils.e("weictor", dataStr);
        Intent intent = new Intent(ACTION_DATA_AVAILABLE);
        intent.putExtra(EXTRA_ADDRESS, address);
        intent.putExtra(EXTRA_UUID, BleUUIDS.CHARACTERS[1]);
    ...
    private class ServicesDiscoveredTask extends TimerTask {
        ...
        void cancelTask() {
            ...
            LogUtils.w(TAG, "Cancel ServicesDiscoveredTask: " + cancel() + ", i=" + i);
        }
        ...
        public void run() {
            switch (i) {
                case 0:
                    ...
                    boolean success = mBleService.enableNotification(address);
//                    LogUtils.i(TAG, "Enable 0x1002 notification: " + success);
                    break;
